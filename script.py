...
def is_right_hand_coordinate_system3(pose):
    ...
    return n.dot(n) == 1 and o.dot(o) == 1 and a.dot(a) == 1 and n.cross(o) == a
...
class SerialLinkRobot:
    ...
    def update_slider_joint(self, change):
        ...
        with self.fig.batch_update():
            ...
            self.fig.data[-2].x, self.fig.data[-2].y, self.fig.data[-2].z = line_n.tolist()
            self.fig.data[-1].x, self.fig.data[-1].y, self.fig.data[-1].z = line_a.tolist()
    ...
    def interact(self, figure_width=800, figure_height=600):
        ...
        widget_box = widgets.VBox([self.toggle_joint, self.slider_joint, fig])
        return widget_box
